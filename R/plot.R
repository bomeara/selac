...
#' @examples
#'
#' psi.vector <- c(0.01, .1, 0.5, 2)
#' eq.freq.matrices <- array(dim=c(64, 20, length(psi.vector)))
#' for (i in sequence(length(psi.vector))) {
#'   eq.freq.matrices[,,i] <- ComputeEquilibriumCodonFrequencies(Psi=psi.vector[i])
#' }
#' values = paste("Psi = ", psi.vector, sep="")
...
#' @param ... Other paramters to pass to plot()
#' @examples
#' psi.vector <- c(0.0000000001, 0.01, .1, 0.5, 2)
#' aa.fitness.matrices <- array(dim=c(21, 20, length(psi.vector)))
#' for (i in sequence(length(psi.vector))) {
#'  local.matrix <- ComputeEquilibriumAAFitness(Psi=psi.vector[i])$aa.fitness.matrix
...
#' @param ... Other paramters to pass to plot()
#' @examples
#' psi.vector <- c(.1, 0.5, 2)
#' codon.fitnesses.matrices <- array(dim=c(64, 20, length(psi.vector)))
#' codon.eq.matrices <- array(dim=c(64, 20, length(psi.vector)))
#' for (i in sequence(length(psi.vector))) {
#'    local.matrix <- ComputeEquilibriumAAFitness(Psi=psi.vector[i])
...
#' for (i in sequence(length(psi.vector))) {
#'    mutation.fitness.object.list[[i]] <- ComputeMutationFitnesses(Psi=psi.vector[i])
#' }
#' PlotMutationFitnessSpectra(mutation.fitness.object.list, values=paste("Psi =", psi.vector), optimal.aa="C")
