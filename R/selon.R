        ...
        for(gen.index in 1:length(generations)){
            #for(index in 1:length(generations[[gen.index]])){
                ...
                if(current.lik > out$objective){
                    ...
                    phy$edge.length[which(phy$edge[,2] %in% generations[[gen.index]])] <- exp(out$minimum)
                }
